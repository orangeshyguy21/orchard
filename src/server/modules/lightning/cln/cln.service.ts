...
export class ClnService {
	...
	private createGrpcCredentials() {
		...
		if (!rpc_host || !rpc_port || !rpc_macaroon || !rpc_tls_cert) {
			this.logger.warn('Missing CLN RPC credentials, secure connection cannot be established');
			return null;
		}
	...
	public initializeLightningClient(): grpc.Client {
		const lightning_proto_path = path.join(process.cwd(), 'proto/cln/lightning.proto');
