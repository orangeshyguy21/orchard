 ...
 *
 *  Skipped when `ORCHARD_E2E_PARALLEL=1` — the parallel runner script
 *  prints the matrix once from the parent and suppresses per-child copies. */

/* Vendor Dependencies */
import type {Reporter, FullConfig, Suite} from '@playwright/test/reporter';

/* Native Dependencies */
import {BOLD, CYAN, DIM, GREEN, MAGENTA, RESET, YELLOW, stripAnsi} from './ansi';
...
/** Pure renderer — given a per-stack test count map (keyed by bare config
 *  name), prints the matrix to stdout. Stacks with `count === 0` are hidden,
 *  so partial runs (`--grep`, `--project`) only show what's scheduled. */
export function printMatrix(counts: Map<string, number>): void {
...
export default class SummaryReporter implements Reporter {
	onBegin(_config: FullConfig, suite: Suite): void {
		if (process.env.ORCHARD_E2E_PARALLEL === '1') return;
